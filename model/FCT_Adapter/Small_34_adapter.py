...
class Transchor(nn.Module):
    def __init__(self, embed_dim, h=8, cat=False):
        ...
        self.depthwise_conv = nn.Conv2d(self.embed_dim, self.embed_dim, kernel_size=3, stride=1, padding=1, groups=self.embed_dim)
...
def set_adapter(model, adapter, embed_dim, f=8, h=8, r=1, s=0.1):
    if adapter == "Convchor":
        for _ in model.modules():
            ...
            if type(_) == timm.models.resnet.BasicBlock:
                ch_in = _.conv1.in_channels
                ch_out = _.conv2.out_channels
                stride = _.conv1.stride
                _.conv_adapter = Convchor(ch_in=ch_in, f=f, ch_out=ch_out, stride=stride, r=r)
                bound_method = forward_conv.__get__(_, _.__class__)
                setattr(_, 'forward', bound_method)
...
class Small_34_adapter(nn.Module):

    def __init__(self, cnn, vit, m=8, a=1, num_classes=100, embed_dim=384):
        # Transformer
        super().__init__()
        ...
        self.adapter1 = Bridge(64, embed_dim=embed_dim, m=m, a=a)
        self.adapter2 = Bridge(64, embed_dim=embed_dim, m=m, a=a)
        self.adapter3 = Bridge(64, embed_dim=embed_dim, m=m,  a=a)
        self.adapter4 = Bridge(128, embed_dim=embed_dim, m=m, a=a)
        self.adapter5 = Bridge(128, embed_dim=embed_dim, m=m, a=a)
        self.adapter6 = Bridge(128, embed_dim=embed_dim, m=m, a=a)
        self.adapter7 = Bridge(128, embed_dim=embed_dim, m=m, a=a)
        self.adapter8 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter9 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter10 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter11 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter12 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter13 = Bridge(256, embed_dim=embed_dim, m=m, a=a)
        self.adapter14 = Bridge(512, embed_dim=embed_dim, m=m, a=a)
        self.adapter15 = Bridge(512, embed_dim=embed_dim, m=m, a=a)
        self.adapter16 = Bridge(512, embed_dim=embed_dim, m=m, a=a)
...
class Resnet34_ViT_S(nn.Module):
    ...
    def __init__(self, num_classes, f, h, m, r, s, a):
        super().__init__()
        self.resnet34 = create_model('resnet34', checkpoint_path='../pre_weight/resnet34.pth', drop_path_rate=0.1)
        ...
        self.model = Small_34_adapter(cnn=self.resnet34, vit=self.vit_model, m=m, a=a, num_classes=num_classes,
                                      embed_dim=embed_dim)
...
if __name__ == '__main__':
    model = Resnet34_ViT_S(num_classes=100, f=8, h=8, m=8, r=1, s=0.1, a=1)
