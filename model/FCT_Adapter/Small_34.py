...
class ConvBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, f=8, r=1, downsample=None, cardinality=1, base_width=64,
        ...
        self.conv_adapter = Convchor(inplanes, f, outplanes, stride, r)
...
class Transchor(nn.Module):
    def __init__(self, h=8, xavier_init=False, cat=False, norm_layer=partial(nn.LayerNorm, eps=1e-6)):
        ...
        self.depthwise_conv = nn.Conv2d(384, 384, kernel_size=3, stride=1, padding=1, groups=384)
        ...
        if cat == False:
            self.adapter_down = nn.Linear(384, h)  # equivalent to 1 * 1 Conv
        else:
            self.adapter_down = nn.Linear(384 * 2, h)  # equivalent to 1 * 1 Conv
        self.adapter_up = nn.Linear(h, 384)  # equivalent to 1 * 1 Conv
        nn.init.xavier_uniform_(self.adapter_down.weight)
        nn.init.zeros_(self.adapter_down.bias)
        nn.init.zeros_(self.adapter_up.weight)
        nn.init.zeros_(self.adapter_up.bias)
        self.layer1 = norm_layer(8)
        self.layer2 = norm_layer(384)
    ...
    def forward(self, x):
        ...
        x_patch = x[:, 1:].reshape(B, 14, 14, 384).permute(0, 3, 1, 2)
        x_cls = x[:, :1]
        ...
        x_patch = x_patch.permute(0, 2, 3, 1).reshape(B, 14 * 14, 384)
...
class Bridge(nn.Module):
    def __init__(self, outplanes, m=8, h=8, a=1, act_layer=nn.GELU, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        # 升维
        self.adapter_up = nn.Conv2d(m, 384, 1, 1, 0)
        nn.init.zeros_(self.adapter_up.weight)
    ...
    def forward(self, x_t, x_r):
        ...
        x_t = x_t[:, 1:].reshape(B, 14, 14, 384).permute(0, 3, 1, 2)
        x_t = x_t + self.a * x_r
        x_t = x_t.permute(0, 2, 3, 1).reshape(B, 14 * 14, 384)
...
class Small_34(nn.Module):
    ...
    def __init__(self, f=8, m=8, h=8, r=1, a=1, s=0.1, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4,
        ...
        self.layer1 = self._make_layer(ConvBlock, 64, 3, f=f,r=r)
        self.layer2 = self._make_layer(ConvBlock, 128, 4, stride=2,f=f,r=r)
        self.layer3 = self._make_layer(ConvBlock, 256, 6, stride=2,f=f,r=r)
        self.layer4 = self._make_layer(ConvBlock, 512, 3, stride=2,f=f,r=r)

        self.adapter1 = Bridge(64, m=m, h=h, a=a)
        self.adapter2 = Bridge(64, m=m, h=h, a=a)
        self.adapter3 = Bridge(64, m=m, h=h, a=a)
        self.adapter4 = Bridge(128, m=m, h=h, a=a)
        self.adapter5 = Bridge(128, m=m, h=h, a=a)
        self.adapter6 = Bridge(128, m=m, h=h, a=a)
        self.adapter7 = Bridge(128, m=m, h=h, a=a)
        self.adapter8 = Bridge(256, m=m, h=h, a=a)
        self.adapter9 = Bridge(256, m=m, h=h, a=a)
        self.adapter10 = Bridge(256, m=m, h=h, a=a)
        self.adapter11 = Bridge(256, m=m, h=h, a=a)
        self.adapter12 = Bridge(256, m=m, h=h, a=a)
        self.adapter13 = Bridge(256, m=m, h=h, a=a)
        self.adapter14 = Bridge(512, m=m, h=h, a=a)
        self.adapter15 = Bridge(512, m=m, h=h, a=a)
        self.adapter16 = Bridge(512, m=m, h=h, a=a)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, f=8, r=1):
        downsample = None

        # 如果步幅大于1或者输入通道数不等于输出通道数，需要添加下采样层
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride,f,r, downsample))
    ...
    def forward(self, x):
        ...
        # 1 ~2
        x_r, x2_at = self.layer1[0](x_r_stem)
...
if __name__ == '__main__':
    ...
    model = Small_34(f=8, m=8, h=8, r=1, a=1, s=0.1, num_classes=100, drop_path_rate=0.1)
    # summary(model, input_size=(1, 3, 224, 224))
    model_dict = model.state_dict()


    def test(model_dict):
        replace_dict = {'blocks.0.norm1': 'blocks0_att.norm1',
