...
def set_Convpass(model, dim=8, s=1, xavier_init=False):
...
class Convpass_model(nn.Module):
    def __init__(self, num_classes, s, xavier_init=False):
        super().__init__()
        model = create_model('vit_base_patch16_224_in21k', checkpoint_path='../pre_weight/ViT-B_16.npz',
                             drop_path_rate=0.1)
        set_Convpass(model, dim=8, s=s, xavier_init=xavier_init)
        self.model = model.reset_classifier(num_classes)
